...
class FixAuditIssuesTests(unittest.TestCase):
    def test_fixes_timezone_offsets_after_confirmation(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch("builtins.input", side_effect=["3", "3", "yes"]),
            ...
            self.assertIn("3 Fix timezone mismatch or missing offset 1 file", _compact(plain_text))
    ...
    def test_fixes_missing_video_timezone_with_creation_date_and_verifies_readback(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch("sys.stdin.isatty", return_value=True),
                patch("builtins.input", side_effect=["3", "1", "yes"]),
                patch("snapsync.actions.fix_audit_issues_writer.subprocess.run") as run,
                patch(
                    "snapsync.actions.fix_audit_issues_writer.extract_metadata",
                    side_effect=[
    ...
    def test_timezone_fix_missing_only_can_use_custom_offset(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch("sys.stdin.isatty", return_value=True),
                patch("builtins.input", side_effect=["3", "2", "+05:30", "yes"]),
                patch("snapsync.actions.fix_audit_issues_writer.subprocess.run") as run,
                patch(
                    "snapsync.actions.fix_audit_issues_writer.extract_metadata",
    ...
    def test_timezone_fix_preview_orders_by_date_time_then_filename(self):
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("Fingerprint", _strip_colors(text))
    ...
    def test_fixes_unknown_devices_per_file_and_exits(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch("sys.stdin.isatty", return_value=True),
                patch("builtins.input", side_effect=["4", "b", "Canon EOS M50", ""]),
                patch("snapsync.actions.fix_audit_issues_writer.subprocess.run") as run,
                redirect_stdout(output),
            ):
                exit_code = run_audit_issue_fix(root, settings)
    ...
    def test_unknown_device_fix_can_use_whatsapp_shortcut(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch("sys.stdin.isatty", return_value=True),
                patch("builtins.input", side_effect=["4", "a"]),
                patch("snapsync.actions.fix_audit_issues_writer.subprocess.run") as run,
                redirect_stdout(output),
            ):
                exit_code = run_audit_issue_fix(root, settings)
    ...
    def test_bulk_date_fix_previews_change_and_highlights_final_date(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch("sys.stdin.isatty", return_value=True),
                patch("builtins.input", side_effect=["1", "1", "26-08-2026", "yes"]),
            ...
            self.assertIn("DRY RUN: no metadata was written.", text)
            self.assertIn("1 Repair all scanned files", _compact(_strip_colors(text)))
    ...
    def test_bulk_time_fix_previews_change_and_highlights_final_time(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch("sys.stdin.isatty", return_value=True),
                patch("builtins.input", side_effect=["1", "2", "08:09:10", "yes"]),
                patch("snapsync.actions.fix_audit_issues_prompts.write_datetime") as write_datetime,
                redirect_stdout(output),
            ):
                exit_code = run_audit_issue_fix(root, settings)
            ...
            self.assertIn("DRY RUN: no metadata was written.", text)
            compact_text = _compact(_strip_colors(text))
            self.assertIn("Filename Old Time New Time", compact_text)
            self.assertIn("wrong-time.jpg 12:00:00 08:09:10", compact_text)
            self.assertIn("wrong-time.jpg 05-01-2026 08:09:10 DateTimeOriginal +02:00 UnknownDevice", compact_text)
    ...
    def test_bulk_timezone_fix_moves_time_and_highlights_time_and_offset(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch("sys.stdin.isatty", return_value=True),
                patch("builtins.input", side_effect=["1", "3", "+05:30", "yes"]),
                patch("snapsync.actions.fix_audit_issues_prompts.write_datetime") as write_datetime,
                redirect_stdout(output),
            ):
                exit_code = run_audit_issue_fix(root, settings)
            ...
            self.assertIn("DRY RUN: no metadata was written.", text)
            compact_text = _compact(_strip_colors(text))
            self.assertIn("Filename Old Date/Time New Date/Time Old Offset New Offset", compact_text)
            self.assertIn(
                "wrong-offset.jpg 05-01-2026 12:00:00 05-01-2026 15:30:00 +02:00 +05:30",
                compact_text,
            )
            self.assertIn(
                "wrong-offset.jpg 05-01-2026 15:30:00 DateTimeOriginal +05:30 UnknownDevice",
                compact_text,
            )
    ...
    def test_batch_date_repair_changes_matching_date_and_device_only(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch("sys.stdin.isatty", return_value=True),
                patch("builtins.input", side_effect=["2", "1", "26-07-2026", "26-08-2026", "iPhone", "yes"]),
                patch("snapsync.actions.fix_audit_issues_prompts.write_datetime") as write_datetime,
                redirect_stdout(output),
            ):
                exit_code = run_audit_issue_fix(root, settings)
            ...
            self.assertIn(
                "IMG_7780.heic 26-07-2026 26-08-2026 09:41:37 +05:30 iPhone 16 Pro",
                _compact(_strip_colors(text)),
            )
    ...
    def test_bulk_datetime_timezone_move_shifts_matching_iphone_files_only(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch("sys.stdin.isatty", return_value=True),
                patch("builtins.input", side_effect=["2", "3", "+03:00", "+05:30", "iPhone", "yes"]),
                patch("snapsync.actions.fix_audit_issues_prompts.write_datetime") as write_datetime,
                redirect_stdout(output),
            ):
                exit_code = run_audit_issue_fix(root, settings)
            ...
            self.assertEqual(exit_code, 0)
            write_datetime.assert_not_called()
            self.assertIn("2 Repair matching audit issues", _compact(_strip_colors(text)))
            ...
            self.assertIn("snapsync > Fix audit issues > Repair matching audit issues > Change timezone offset", text)
            self.assertIn(
                "IMG_7780.heic 26-08-2026 09:41:37 26-08-2026 12:11:37 +03:00 +05:30",
                _compact(_strip_colors(text)),
            )
            self.assertNotIn("CanonEOS700D", text)
            self.assertIn(
                "IMG_7780.heic 26-08-2026 12:11:37 DateTimeOriginal +05:30 iPhone 16 Pro",
                _compact(_strip_colors(text)),
            )
    ...
    def test_bulk_device_fix_previews_change_and_highlights_final_device(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch("sys.stdin.isatty", return_value=True),
                patch("builtins.input", side_effect=["1", "4", "b", "Canon EOS R50", "yes"]),
            ...
            self.assertIn("DRY RUN: no metadata was written.", text)
            compact_text = _compact(_strip_colors(text))
            self.assertIn("Filename Old Device New Device", compact_text)
            self.assertIn("wrong-device.jpg UnknownDevice Canon EOS R50", compact_text)
            self.assertIn("wrong-device.jpg 05-01-2026 12:00:00 DateTimeOriginal +02:00 Canon EOS R50", compact_text)
